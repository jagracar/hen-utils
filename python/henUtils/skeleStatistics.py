...
from tezos import build_castle, have_fun
from cryptoclimate import random_wave
...
while community > 0:
    if castle is None:
        castle = build_castle(community)

    community += have_fun(community, castle)
...
bids_bigmap = get_objktcom_bigmap(
    "bids", "skele", transactions_dir, sleep_time=1)
asks_bigmap = get_objktcom_bigmap(
    "asks", "skele", transactions_dir, sleep_time=1)
english_auctions_bigmap = get_objktcom_bigmap(
    "english auctions", "skele", transactions_dir, sleep_time=1)
dutch_auctions_bigmap = get_objktcom_bigmap(
    "dutch auctions", "skele", transactions_dir, sleep_time=1)

# Select only the bids and asks transactions related with skeles
...
# Get the skele ledger bigmap
ledger_bigmap = get_token_bigmap("ledger","skele", transactions_dir, sleep_time=1)

token_owners = {}

for token_id, wallet in ledger_bigmap.items():
    if wallet.startswith("tz"):
        if wallet in token_owners:
            token_owners[wallet].append(token_id)
        else:
            token_owners[wallet] = [token_id]

items = np.array([len(tokens) for tokens in token_owners.values()])
token_wallets = np.array([wallet for wallet in token_owners.keys()])

sorted_indices = np.argsort(items)[::-1]
token_wallets = token_wallets[sorted_indices]
...
for i, wallet in enumerate(token_wallets[:100]):
    if wallet in registries_bigmap:
        print(" %3i: Collector %s has %4i SKELEs (%s)" % (
            i + 1, wallet, items[i], registries_bigmap[wallet]["user"]))
    else:
        print(" %3i: Collector %s has %4i SKELEs" % (
            i + 1, wallet, items[i]))

# Plot the number of operations per day
plot_operations_per_day(
    bid_transactions, "SKELE objkt.com bid operations per day",
    "Days since first minted SKELE (31st of October)", "Bid operations per day",
    exclude_last_day=exclude_last_day, first_month=10, first_day=31)
save_figure(os.path.join(figures_dir, "skele_bid_operations_per_day.png"))

plot_operations_per_day(
    ask_transactions, "SKELE objkt.com ask operations per day",
    "Days since first minted SKELE (31st of October)", "Ask operations per day",
    exclude_last_day=exclude_last_day, first_month=10, first_day=31)
save_figure(os.path.join(figures_dir, "skele_ask_operations_per_day.png"))

plot_operations_per_day(
    english_auction_transactions, "SKELE objkt.com english auction operations per day",
    "Days since first minted SKELE (31st of October)",
    "English auction operations per day", exclude_last_day=exclude_last_day,
    first_month=10, first_day=31)
save_figure(os.path.join(figures_dir, "skele_english_auction_operations_per_day.png"))

#plot_operations_per_day(
#    dutch_auction_transactions, "SKELE objkt.com dutch auction operations per day",
#    "Days since first minted SKELE (31st of October)",
#    "Dutch auction operations per day", exclude_last_day=exclude_last_day,
#    first_month=10, first_day=32)
#save_figure(os.path.join(figures_dir, "skele_dutch_auction_operations_per_day.png"))
...
print("There are currently %i unique SKELE collectors." % len(collectors))
...
plot_histogram(
    ...
    title="SKELE collectors",
    x_label="Total money spent (tez)", y_label="Number of collectors", bins=100)
save_figure(os.path.join(figures_dir, "skele_top_collectors_histogram.png"))
...
plot_data_per_day(
    ...
    "Money spent per day",
    "Days since first minted SKELE (31st of October)", "Money spent (tez)",
    exclude_last_day=exclude_last_day, first_month=10, first_day=31)
save_figure(os.path.join(figures_dir, "skele_money_per_day.png"))
